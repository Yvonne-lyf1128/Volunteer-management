...
const DEFAULT_DATA = {
  ...
  hoursSubmissionOpen: false  // 默认关闭，管理员需要手动开启
};
...
app.get('/api/settings', (req, res) => {
  res.json({ hoursSubmissionOpen: db.hoursSubmissionOpen !== undefined ? db.hoursSubmissionOpen : false });
});
...
app.post('/api/volunteer/:id/hours', (req, res) => {
  const vol = db.volunteers.find(v => v.id === req.params.id);
  if (!vol) return res.status(404).json({ error: '志愿者不存在' });
  if (!db.hoursSubmissionOpen) return res.status(403).json({ error: '管理员尚未开放时长提交，请等待通知' });
...
// 管理员切换时长提交开关
app.post('/api/admin/toggle-hours', (req, res) => {
  db.hoursSubmissionOpen = !db.hoursSubmissionOpen;
  saveData();
  res.json({ success: true, hoursSubmissionOpen: db.hoursSubmissionOpen });
});

// 管理员重置志愿者密码
app.post('/api/admin/volunteer/:id/reset-password', (req, res) => {
  const vol = db.volunteers.find(v => v.id === req.params.id);
  if (!vol) return res.status(404).json({ error: '志愿者不存在' });
  const { newPassword } = req.body;
  if (!newPassword || newPassword.length < 4) return res.status(400).json({ error: '新密码至少4位' });
  vol.password = newPassword; saveData();
